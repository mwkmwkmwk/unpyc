...
def ast_process(deco, version):
    ...
    def process_1(node):
        ...
        if isinstance(node, ExprCall) and isinstance(node.expr, ExprBuildClass):
            args = node.args.args
            if not (len(args) >= 2
                and isinstance(args[0], CallArgPos)
                and isinstance(args[1], CallArgPos)
                and isinstance(args[0].expr, ExprFunctionRaw)
                and isinstance(args[1].expr, ExprUnicode)
            ):
                raise PythonError("funny args to $buildclass")
            name = args[1].expr.val
            fun = args[0].expr
    ...
    def process_block_2(node):
        for idx, stmt in enumerate(node.stmts):
            ...
            if isinstance(stmt, StmtExceptDead):
                ...
                any(_end_continue(item.body) for item in stmt.items)):
    ...
    def undecorate(node):
        ...
        while isinstance(node, ExprCall):
            if len(node.args.args) != 1:
                return None, None
            if not isinstance(node.args.args[0], CallArgPos):
                return None, None
            if not isdecorator(node.expr):
                return None, None
            decorators.append(node.expr)
            node = node.args.args[0].expr
    ...
    def process_2(node):
        ...
        if isinstance(node, StmtJunk):
            return process_if(StmtIf([IfItem(ExprAnyTrue(), Block([]))], node.body))
    ...
    def process_4(node):
        ...
        if isinstance(node, StmtIfRaw):
            return process_if(StmtIf([IfItem(node.cond, node.body)], node.else_))
        if isinstance(node, StmtIfDead):
            return StmtIf([IfItem(node.cond, node.body)], None)
